...
import { LogOut, User, Pencil, KeyRound, Mail, Phone, MapPin, Calendar, Globe, Shield, Check } from 'lucide-react-native';
...
export default function TeacherProfile() {
  ...
  const [editForm, setEditForm] = useState({ full_name: '', phone: '', address: '', gender: '', date_of_birth: '', nationality: '' });
  const [passwordForm, setPasswordForm] = useState({ newPassword: '', confirmPassword: '' });

  useEffect(() => { supabase.auth.getSession().then(({ data: { session } }) => { if (session?.user?.email) setEmail(session.user.email); }); }, []);
  useEffect(() => { if (editOpen && profile) setEditForm({ full_name: profile.full_name ?? '', phone: profile.phone ?? '', address: profile.address ?? '', gender: profile.gender ?? '', date_of_birth: profile.date_of_birth?.split('T')[0] ?? '', nationality: profile.nationality ?? '' }); }, [editOpen, profile]);
  ...
  const saveProfile = async () => {
    ...
    setSaving(true);
    const { error } = await supabase.from('app_users').update({ full_name: editForm.full_name.trim(), phone: editForm.phone || null, address: editForm.address || null, gender: editForm.gender || null, date_of_birth: editForm.date_of_birth || null, nationality: editForm.nationality || null }).eq('id', profile.id);
    setSaving(false);
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      ...
      <Card style={{ marginTop: 14 }}>
        ...
        <InfoRow icon={<Calendar color={colors.muted} size={16} />} label="Date of Birth" value={formatDate(profile?.date_of_birth)} colors={styles ? colors : colors} />
        <InfoRow icon={<Globe color={colors.muted} size={16} />} label="Nationality" value={profile?.nationality ?? '—'} colors={colors} />
        <InfoRow icon={<Shield color={colors.muted} size={16} />} label="Qualification" value={profile?.qualification ?? '—'} colors={colors} />
      ...
      <Modal visible={editOpen} animationType="slide" onRequestClose={() => setEditOpen(false)}>
        <View style={{ flex: 1, backgroundColor: colors.bg }}>
          ...
          <ScrollView contentContainerStyle={{ padding: 20 }}>
            ...
            <Field label="Date of Birth" value={editForm.date_of_birth} onChangeText={(v) => setEditForm({ ...editForm, date_of_birth: v })} placeholder="YYYY-MM-DD" />
            <Field label="Nationality" value={editForm.nationality} onChangeText={(v) => setEditForm({ ...editForm, nationality: v })} placeholder="Enter your nationality" />
